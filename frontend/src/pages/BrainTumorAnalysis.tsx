...
const BrainTumorAnalysis: React.FC = () => {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    if (file) {
      ...
      try {
        ...
        const response = await axios.post('/api/analyze/brain-tumor', formData, {
          headers: {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-medical-primary">Brain Tumor Analysis</h1>
      </div>
            ...
            <div className="mt-6 animate-scale-in">
              <div className="relative">
                ...
                {result?.regions?.map((region, index) => (
                  <div
                    key={index}
                    className="absolute border-2 border-medical-error animate-pulse"
                    style={{
                      left: `${region.x}%`,
                      top: `${region.y}%`,
                      width: `${region.width}%`,
